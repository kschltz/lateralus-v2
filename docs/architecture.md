...
│  :lateralus/factory-session   ──▶  RuntimeToolStore (SCI sandbox with secrets)│
...
│  :lateralus/secret-plugin     ──▶  capability-gated handles + redact         │
...
- **Workspace file index (opt-in):** `:lateralus/store` (`StoreEngine`, `:memory` or `:duckdb`) plus `:lateralus/file-index` (`FileIndex`). Wire `:file-index` into `:lateralus/file-tools` to record SHA-256 witnesses after mutations and to let `file_search` hit stored content when the tree is indexed. The same store can back `:lateralus/session-store` and a historic `:lateralus/stream-bus` (`:impl :store`). See [`file-index.md`](file-index.md). The interceptor chain is unchanged.
