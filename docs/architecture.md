...
│  :lateralus/memory-backend ──▶ MemoryBackend protocol (noop)       │
│  :lateralus/plugins     ──▶  plugin maps (currently [])           │
...
- **New memory backend:** implement `kschltz.agent.memory.protocol/MemoryBackend` and add a case in `kschltz.agent.system/init-key :lateralus/memory-backend`.
