...
- MVP ships a JVM distributable (launcher script or uberjar). GraalVM native-image is a stretch target (Step 9); if Datalevin JNI blocks native-image, MVP completes with documented JVM fallback and a tracked native-image blocker issue.
...
- Out of MVP scope: web-search, remember, file editing (clj_edit/file_edit), repl-eval, Portal visualize, nREPL integration, and v1 uberjar build.
