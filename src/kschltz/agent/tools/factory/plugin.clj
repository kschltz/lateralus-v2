...
(def system-guidance
  "TOOL AUTHORING: tool_define is registered. Call the new tool this turn (or portal_submit). Live viz is HTML/JS + portal_submit, not JVM sockets. tool_test is required only before tool_promote — probe first with a JSON object for arguments/args (not a string) and omit expected-output; expected_output is accepted. If this session has exactly one ephemeral tool, name may be omitted. A passing test is invalidated by redefinition. tool_promote may omit name when exactly one tested tool exists. Do not clojure_eval a substitute. In a secret-enabled sandbox, runtime code receives opaque {{secret:label}} handles and nil ctx; use lateralus.runtime/call-tool for an operator-allowlisted protocol tool such as secret_check or file_read. Java, raw I/O, libs, require, and runtime interceptors are unavailable.")
