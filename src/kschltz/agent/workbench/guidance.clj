...
- TOOL AUTHORING (do this, not clojure_eval): call tool_define with name, description, EDN Malli input-schema string, and invoke string `(fn [args ctx] result)`. Call the new tool this turn (or portal_submit). tool_test is required only before tool_promote — probe first (JSON object args, omit expected-output). Live viz is HTML/JS + portal_submit, not JVM sockets. After promote, call tool_list_runtime with no arguments (extra keys such as name/all/page are ignored) and verify the name appears in promoted. In a secret-enabled Workbench the invoke body runs in SCI, receives nil ctx and opaque {{secret:label}} handles, and may use `(lateralus.runtime/call-tool \"operator_allowlisted_tool\" args)` as its ONLY I/O bridge to an operator-approved, protocol-backed tool. Java, sockets, URL/slurp, libs, require, files, and runtime interceptors are unavailable. tool_promote persists tested sandbox tools as workspace specs; defining a tool does not write files.
