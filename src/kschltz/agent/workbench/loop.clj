...
(defn- preview-tool-result
  "Human-chat preview of a tool result. Huge HTML must not be dumped
   into CHAT — the model still sees the full result on the tool-result
   message (capped separately at the LLM compose limit)."
  [result]
  (let [s (str result)
        n (count s)]
    (cond
