...
(deftest handler-secrets-api
  ...
  (testing "PUT stores the secret and the value never echoes back"
    (let [handler (handler-with-secrets)
          res (handler {:request-method :put :uri "/api/secrets"
                        :body (json/generate-string {:label "ui-api-key" :value "hunter2-secret-42"})})
  ...
  (testing "DELETE removes it"
    (let [handler (handler-with-secrets)
          res (handler {:request-method :delete :uri "/api/secrets?label=ui-api-key"})
          body (json/parse-string (:body res) true)]
      (is (= 200 (:status res)))
      (is (:ok body))
...
(deftest handler-secrets-validation
  (let [handler (handler-with-secrets)]
    (testing "invalid label rejected with 400"
      (let [res (handler {:request-method :put :uri "/api/secrets"
                          :body (json/generate-string {:label "bad label!" :value "v"})})]
        (is (= 400 (:status res)))
        (is (str/includes? (:body res) "invalid label"))))
    (testing "blank value rejected"
      (let [res (handler {:request-method :put :uri "/api/secrets"
                          :body (json/generate-string {:label "ok" :value ""})})]
        (is (= 400 (:status res))))))
